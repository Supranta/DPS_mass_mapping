import torch
import numpy as np
import sys
from denoising_diffusion_pytorch import Unet, GaussianDiffusion, Trainer

tomo = bool(int(sys.argv[1]))
print(torch.cuda.is_available())

try:
    schedule = sys.argv[2]
    print("Using %s beta schedule"%(schedule))
except:
    schedule = 'sigmoid'
if(tomo):
    print("Training tomographic model....")
    N_channels = 3
    data_folder = '/home2/supranta/PosteriorSampling/data/Columbia_lensing/MassiveNuS/kappa_128_3bins/'
    KAPPA_MIN = np.array([-0.03479804,-0.05888689,-0.08089042])[:,np.newaxis,np.newaxis]
    KAPPA_MAX = np.array([0.4712809,  0.58141315, 0.6327746])[:,np.newaxis,np.newaxis]

else:
    print("Training single bin model....")
    N_channels = 1
    data_folder = '/home2/supranta/PosteriorSampling/data/Columbia_lensing/MassiveNuS/kappa_128/'

exp_transform = False
schedule_fn_kwargs = dict()
beta_schedule = schedule
if(schedule=='sigmoid'):
    results_folder = './results_deep_model'
    dim_mults = (1, 2, 4, 8, 8)
if(schedule=='sigmoid2'):
    results_folder = './results_sigmoid2'
    beta_schedule = 'sigmoid'
    schedule_fn_kwargs = {'start': -5}
    dim_mults = (1, 2, 4, 8)
if(schedule=='exp_transform'):
    results_folder = './results_exp_transform'
    beta_schedule = 'sigmoid'
    schedule_fn_kwargs = {'start': -5}
    exp_transform = True
    dim_mults = (1, 2, 4, 8)
elif(schedule=='exp_transform1'):
    results_folder = './results_exp_transform1'
    beta_schedule = 'sigmoid'
    schedule_fn_kwargs = {'start': -5.5}
    exp_transform = True
    dim_mults = (1, 2, 4, 8, 8)
elif(schedule=='cosine'):
    results_folder = './results_cosine'
    dim_mults = (1, 2, 4, 8)
elif(schedule=='linear'):
    results_folder = './results_linear'
    dim_mults = (1, 2, 4, 8)

#Adjust model architecture 
model = Unet(
    dim = 64,
    dim_mults = dim_mults,
    flash_attn = False, 
    channels = N_channels
).cuda()


diffusion = GaussianDiffusion(
    model,
    image_size = 128,
    timesteps = 1000,    # number of steps
    sampling_timesteps = 999,
    beta_schedule = beta_schedule,
    schedule_fn_kwargs = schedule_fn_kwargs,
    kappa_min = KAPPA_MIN,
    kappa_max = KAPPA_MAX,
    exp_transform = exp_transform
).cuda()

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

total_params = count_parameters(diffusion)
print(f'Total trainable parameters: {total_params:,}')

for name, param in diffusion.named_parameters():
    if param.requires_grad:
        print(f'{name}: {param.numel():,} parameters')

#Adjust training specifications
trainer = Trainer(
    diffusion,
    data_folder, 
    train_batch_size = 32,
    train_lr = 8e-7,
    save_and_sample_every = 10000,
    train_num_steps = 500000,         # total training steps
    gradient_accumulate_every = 2,    # gradient accumulation steps
    ema_decay = 0.995,                # exponential moving average decay
    amp = True,                       # turn on mixed precision
    calculate_fid = False,            # whether to calculate fid during training
    exp_transform = exp_transform,
    results_folder = results_folder
)
trainer.load('33')

trainer.train()






